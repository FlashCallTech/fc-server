					...
					<div className="overflow-auto py-2 px-4 text-sm border-[1px] rounded-lg shadow-[0px_1px_2px_0px_rgba(0,0,0,0.05)] bg-gradient-to-t from-[rgba(0,0,0,0.001)] to-[rgba(0,0,0,0.001)] text-[#6B7280]">
						...
						<table className="w-full text-left">
							...
							<tbody>
								{userCalls?.pages?.flatMap((page: any) =>
									page?.calls?.map((userCall: RegisterCallParams) => {
										const formattedDate = formatDateTime(userCall.startedAt as Date);
										const creator = userCall.expertDetails;
										const fullName = getDisplayName(creator);
										...
										return (
											<tr key={userCall.callId} className="bg-white border-b ">
												...
												<td className="px-6 py-4">
													<div className={`flex gap-3 items-center ${userCall.status !== "Ended" ? "text-red-500" : ""}`}>
														{userCall.type === "chat" ? (
															<svg xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" fill="none" version="1.1" width="100" height="100" viewBox="0 0 20 20" className="size-4">
																<defs>
														...
														{userCall.type === "audio" ? "Audio" : userCall.type === "video" ? "Video" : "Chat"}
													</div>
												</td>
												<td className="px-6 py-4">
													{formatDuration(parseInt(userCall?.duration as string, 10)) ?? "-"}
												</td>
												<td className="px-6 py-4">
													{userCall.global && userCall.amountINR ? `₹ ${userCall.amountINR.toFixed(2)}` :
														!userCall.global && userCall.amount ? `₹ ${userCall.amount.toFixed(2)}` : "-"}
												</td>
