...
const MeetingRoom = () => {
	...
	useWarnOnUnload("Are you sure you want to leave the meeting?", () => {
		const callId = call?.id || "unknown_meeting";
		const callType = call?.type || "default";

		const callData = {
			call_id: callId,
		};

		const url = `${backendBaseUrl}/official/call/end/${callId}?type=${callType}`;

		try {
			const blob = new Blob([JSON.stringify(callData)], {
			...
			console.error("Error sending call end data:", error);
		}
	});
