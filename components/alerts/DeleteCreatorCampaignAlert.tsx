...
const DeleteCreatorCampaignAlert = ({
	showDeleteCampaignAlert,
	setShowDeleteCampaignAlert,
	handleConfirmRemove,
	loading,
	campaignId,
}: {
	showDeleteCampaignAlert: boolean;
	setShowDeleteCampaignAlert: React.Dispatch<React.SetStateAction<boolean>>;
	handleConfirmRemove: (campaignId: string) => Promise<void>;
	loading: boolean;
	...
	const handleRemove = async () => {
		if (campaignId) {
			await handleConfirmRemove(campaignId);
		} else {
			toast({
				variant: "destructive",
				title: "Unable to Remove Campaign",
	...
	return isMobileView ? (
		<Sheet
			open={showDeleteCampaignAlert}
			onOpenChange={setShowDeleteCampaignAlert}
		>
			<SheetContent side="bottom" className=" bg-white rounded-t-xl ">
				<SheetHeader>
					...
					<SheetDescription className="!text-start">
						Are you sure you want to remove Campaign_{campaignId} ?
					</SheetDescription>
				</SheetHeader>
				<div className="w-full flex items-center justify-start gap-2 mt-7">
					<Button
						...
						onClick={() => setShowDeleteCampaignAlert(false)}
					>
						Cancel
					</Button>
					<Button
...
export default DeleteCreatorCampaignAlert;
