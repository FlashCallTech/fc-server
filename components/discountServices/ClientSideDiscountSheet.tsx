...
const ClientSideDiscountheet = ({
	...
	let discount = selectedService ?? newUserService;
	...
	useEffect(() => {
		...
		const seenDiscountheet = sessionStorage.getItem("hasSeenDiscountheet");

		if (!seenDiscountheet && userServices.length > 0) {
			setIsOpen(true);
			sessionStorage.setItem("hasSeenDiscountheet", "true");
		}
