...
const HelpChat = () => {
          ...
          if (userHelpChatsSnapshot.exists()) {
            ...
            if (chatIndex !== -1) {
              ...
              userChatsData.chats[chatIndex].isSeen =
                id === (currentUser?._id ?? temporaryClientId);
  ...
  const handleEnd = () => {
    setLoading(true);
    router.replace(
      `${userType === "client" && creatorURL ? `${creatorURL}` : "/"}`
    );
  };
  ...
  return (
    <div className="size-full">
      ...
      >
        ...
        {/* Messages */}
        <main
          className={`overflow-y-auto scrollbar-hide p-2 gap-2 flex flex-col-reverse`}
        >
          {chat?.messages &&
            [...chat.messages].reverse().map((msg: any, index, arr) => {
              ...
              return (
                <React.Fragment key={msg.createdAt + "-" + index}>
                  <div
                    className={`flex ${
                      msg.senderId === (currentUser?._id ?? temporaryClientId)
                        ? "justify-end"
                        : "justify-start"
                    }`}
                  >
                    <div
                      className={`px-4 py-2 text-sm shadow-sm rounded-2xl max-w-[70%] ${
                        msg.senderId === (currentUser?._id ?? temporaryClientId)
                          ? "text-black bg-white"
                  ...
                  {/* Place ref after the latest message */}
                  {showSeparator && (
      ...
      >
        <div className="md:w-[50%] lg:w-[70%] h-[98%] md:flex flex-col rounded-md bg-gray-100">
                  ...
                  <div className="mb-4 text-left">
                    {chat?.messages?.map((message: Messages, index: number) => {
                      const isCurrentUserMessage =
                        message.senderId ===
                        ((currentUser?._id ?? temporaryClientId) as string);
