...
const PaymentSettings = () => {
  ...
  useEffect(() => {
    const fetchPaymentDetails = async () => {
      ...
      try {
        const response = await fetch(`/api/v1/creator/get-payment?userId=${currentUser?._id}`, {
  ...
  const isValidUpiId = useCallback((upiId: string) => /^[\w.-]+@[\w.-]+$/.test(upiId), []);
  const isValidIfscCode = useCallback((ifscCode: string) => /^[A-Z]{4}0[A-Z0-9]{6}$/.test(ifscCode), []);
  const isValidAccountNumber = useCallback((accountNumber: string) => /^\d{9,18}$/.test(accountNumber), []);

  const handleSave = async () => {
    let hasError = false;
    const newErrors = { upiId: "", ifscCode: "", accountNumber: "", accountType: "" };
  ...
  return (
    <div className="flex flex-col gap-4 bg-gray-100 h-full mx-auto p-4">
      <div className="flex items-start">
        <button
          onClick={() => router.back()}
          className="text-xl font-bold l-0"
        >
          &lt;
        </button>
      </div>
      <div className="flex flex-col justify-between bg-gray-100 h-full">
        <div>
          ...
          {paymentMethod === "BankTransfer" && (
            <>
              <div className="mb-4">
                <label className="block text-sm font-semibold mb-1" htmlFor="ifscCode">IFSC Code</label>
                <input
                  id="ifscCode"
                  type="text"
