...
const KycForm: React.FC = () => {
	...
	useEffect(() => {
		if (creatorUser?.kycStatus === "COMPLETED") {
			setKycDone(true);
			return;
		}
		const getKyc = async () => {
			const response = await fetch(
				`/api/v1/userkyc/getKyc?userId=${currentUser?._id}`,
				{
					method: "GET",
					headers: {
	...
	const generateVerificationId = () => {
		return `${currentUser?._id}_${Date.now()}_${Math.random()
			.toString(36)
			.substr(2, 9)}`;
	};
	...
	return (
		<div className="flex flex-col items-center justify-center h-full w-full">
			<button
				className="rounded bg-black text-white font-bold p-4 mb-4 hover:cursor-pointer"
				onClick={handleKyc}
				disabled={formLink !== undefined ? true : false}
			>
				Start Your Kyc
			</button>
