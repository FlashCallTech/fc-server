		...
		<div className="relative size-full mx-auto py-4 px-1.5">
			...
			<form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
				{fields.map((dayItem, dayIndex) => (
					<Card key={dayItem.id} className="shadow-md">
						<CardHeader>
							<div className="flex justify-between items-center">
								<div className="flex items-center space-x-2">
									<Checkbox
										checked={watch(`weeklyAvailability.${dayIndex}.isActive`)}
									...
									/>
									<CardTitle className="text-lg">{dayItem.day}</CardTitle>
								</div>
						...
						<CardContent>
							{watch(`weeklyAvailability.${dayIndex}.slots`)?.map(
								(slot, slotIndex) => (
									<div key={slot.id} className="flex items-center gap-4 mb-2">
										...
										{(watch(`weeklyAvailability.${dayIndex}.slots`)?.length ||
											0) > 1 && (
											<Button
												type="button"
												variant="destructive"
							...
							{Array.isArray(errors.weeklyAvailability?.[dayIndex]?.slots) &&
								errors.weeklyAvailability[dayIndex].slots.map(
									(slotError, slotIndex) =>
										slotError?.endTime && (
											<p key={slotIndex} className="text-red-500 text-sm">
