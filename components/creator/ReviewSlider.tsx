		...
		<>
			<Slider {...settings} ref={sliderRef} className="py-4">
				{creatorFeedbacks.map((feedback, index) => {
					...
					const fullName = feedback.isCustomAdded
						? feedback.customUserName || "Custom User"
						: getDisplayName(feedback.clientId);

					const userImage = feedback.isCustomAdded
						? feedback.customUserImage || "/images/defaultProfileImage.png"
						: feedback.clientId?.photo && isValidUrl(feedback.clientId.photo)
