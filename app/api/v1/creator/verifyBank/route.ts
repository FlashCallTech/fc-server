		...
		if (result.account_status === "VALID") {
			...
			if (beneficiary === null) {
				const getUserResponse = await fetch(
					"https://flashcall.me/api/v1/creator/getUserById",
					{
						method: "POST",
						headers: {
							"Content-Type": "application/json",
				...
				if (user) {
					...
					if (beneficiaryResult.beneficiary_status === "VERIFIED") {
						...
						const payload = {
							...
							beneficiary_instrument_details: {
								bank_account_number:
									beneficiaryResult.beneficiary_instrument_details
										.bank_account_number,
								bank_ifcs:
									beneficiaryResult.beneficiary_instrument_details.bank_ifsc,
							},
							beneficiary_contact_details: beneficiary_contact_details,
							added_on: beneficiaryResult.added_on,
						};
						const postBeneficiaryResponse = await fetch(
							"https:/flashcall.me/api/v1/beneficiary/postBeneficiary",
							{
								method: "POST",
								headers: {
									"Content-Type": "application/json",
						...
						await fetch("https://flashcall.me/api/v1/creator/updateUser", {
							method: "PUT",
							headers: {
								"Content-Type": "application/json",
							},
				...
				const getBeneficiaryResponse = await fetch(
					`https://api.cashfree.com/payout/beneficiary?beneficiary_id=${userId}`,
					{
						method: "GET",
						headers: {
							"Content-Type": "application/json",
