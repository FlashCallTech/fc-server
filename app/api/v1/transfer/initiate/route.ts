...
export async function POST(request: NextRequest) {
	try {
		const { userId, phone, amount } = await request.json();
		...
		if (amount >= 50) {
			if (userResponse[0].walletBalance >= amount) {
				...
				if (paymentSettingsResult.success) {
					...
					if (kycResult.data.kyc_status === "COMPLETED") {
						const getBeneficiary = await fetch(
							`https://flashcall.me/api/v1/beneficiary/getBeneficiary?userId=${userId}`,
							{
								method: "GET",
								headers: {
									"Content-type": "application/json",
						...
						if (getBeneficiaryResult.success) {
							...
							const response = await fetch(
								`https://api.cashfree.com/payout/transfers?transfer_id=${initiateResult.transfer_id}`,
								{
									method: "GET",
									headers: {
										'x-client-id': process.env.NEXT_PUBLIC_CASHFREE_CLIENT_ID_Payout as string, // Replace with your client ID
							...
							if (result.status === "SUCCESS") {
								...
								const payout = await fetch(
									"https://flashcall.me/api/v1/wallet/payout",
									{
										method: "POST",
										body: JSON.stringify({
											userId: userId,
								...
								if (result.status === "FAILED")
									return NextResponse.json({
										success: false,
										message: "IMPS_MODE_FAILED",
									});
								else if (result.status === "REJECTED")
									return NextResponse.json({
										success: false,
										data: result,
										message: "INVALID_MODE_FOR_PYID",
									});
				...
				return NextResponse.json({
					success: false,
					message: "Entered amount is greator than wallet balance",
				});
			}
		} else {
			return NextResponse.json({
				success: false,
				message: "Minimum withdraw amount required is 50",
			});
		}
	} catch (error) {
		return NextResponse.json({
