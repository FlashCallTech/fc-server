  ...
  try {
    ...
    if (amount > 0) {
      if (userResponse[0].walletBalance >= amount) {
        ...
        return NextResponse.json({ success: false, message: 'Entered amount is greator than wallet balance' });
      }
    } else {
      return NextResponse.json({ success: false, message: 'Minimum withdraw amount required is 500' });
    }
  } catch (error) {
