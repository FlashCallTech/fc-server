...
export async function POST(request: NextRequest) {
  try {
    ...
    const { verificationId, first_img, second_img, userId } = await request.json();
    ...
    // Convert base64 string (second image) to a Blob
    ...
    const blobAadhaar = await base64Response.blob();

    // Prepare form data for the API request
    const formData = new FormData();
    formData.append('first_image', blobLiveliness);
    formData.append('second_image', blobAadhaar);
    formData.append('verification_id', verificationId);

    // Make a POST request to the Cashfree Face Match API
    const response = await fetch('https://api.cashfree.com/verification/face-match', {
      method: 'POST',
      body: formData,
      headers: {
