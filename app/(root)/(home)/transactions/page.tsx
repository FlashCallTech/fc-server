    ...
    <section className="size-full grid grid-cols-1 grid-rows-[auto,auto,1fr]">
      ...
      <ul className="space-y-4 w-full px-4 pt-2 pb-7">
        {!isLoading || !currentUser ? (
          isError ? (
            <div className="size-full h-[calc(100vh-6rem)] flex flex-col items-center justify-center text-2xl xl:text-2xl font-semibold text-center text-red-500">
              Failed to fetch Transactions
              <span className="text-lg">Please try again later.</span>
            </div>
          ) : Object.keys(groupedTransactions).length === 0 ? (
            <section
              className={`size-full h-[calc(100vh-6rem)] flex flex-col gap-4 items-center justify-center text-xl font-semibold text-center text-gray-500`}
            >
              <Image
