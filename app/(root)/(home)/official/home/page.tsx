...
import CreatorHome from "@/components/official/CreatorHome";
...
const OfficialHomePage = () => {
	...
	const handleCreatorCardClick = async (
		...
		setLoadingCard(true);
		updateCreatorURL(`/official/creator/${username}`);
		localStorage.setItem("creatorURL", `/official/creator/${username}`);
		...
		router.push(`/official/creator/${username}`);
	};
