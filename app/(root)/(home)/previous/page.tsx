			...
			<section className={`${userType === "creator" ? "lg:flex" : "hidden"} hidden  size-full flex-col`}>
				<div className="flex w-full gap-2 p-8 pb-2 items-center justify-end">
					{options.map((option) => (
						<Button
							key={option}
							className={`text-sm font-medium px-4 py-2 text-[#6B7280] rounded-full ${historyType === option ? "bg-black text-white" : "hoverScaleDownEffect bg-white text-[#4B5563] border-[1px] border-solid border-[#E5E7EB]"}
								}`}
							onClick={() => setHistoryType(option)}
						>
							{option}
