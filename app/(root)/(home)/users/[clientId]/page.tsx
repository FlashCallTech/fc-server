...
const UserProfilePage: React.FC = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${backendBaseUrl}/calls/clientData/${clientId}/${currentUser?._id}`);
    ...
    const getButtonClasses = (tab: Tab) =>
        `pb-4 px-1 cursor-pointer ${selectedButton === tab
    ...
    const lastSessionDate =
        calls && calls.length > 0 ? calls[0].startedAt : null;
            ...
            <div className="size-full border rounded-lg">
                ...
                <div className="flex gap-2 pl-28 pr-6 text-sm pb-8 border-b">
                    ...
                    <div className="flex-1 flex flex-col gap-1 bg-gray-100 p-4 rounded-lg">
                        ...
                        <span className="font-semibold text-lg">
                            {lastSessionDate ? format(new Date(lastSessionDate), "MMM d") : "N/A"}
                        </span>
                    </div>
                </div>
                {/* Tabs */}
                <div className="border-b px-6 py-4">
                    <nav className="flex gap-9 text-sm text-[#6B7280]">
                        <button onClick={() => setSelectedButton("Overview")} className={getButtonClasses("Overview")}>
                            Overview
                        </button>
                        <button onClick={() => setSelectedButton("Sessions")} className={getButtonClasses("Sessions")}>
                            Sessions
                        </button>
                        <button onClick={() => setSelectedButton("Messages")} className={getButtonClasses("Messages")}>
                            Messages
                        </button>
                        <button onClick={() => setSelectedButton("Notes")} className={getButtonClasses("Notes")}>
                            Notes
                        </button>
                    </nav>
                </div>
