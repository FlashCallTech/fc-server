	...
	return (
		<div className="size-full flex bg-white">
			<div className="w-full p-8">
				...
				>
					<span className="text-xl text-white font-bold">Invite your friends and earn ₹100 per referral</span>
					<button
						className="flex gap-2 items-center justify-between px-6 py-3 hoverScaleDownEffect"
						style={{
							borderRadius: "12px",
							opacity: 1,
							background: "linear-gradient(90deg, rgba(255, 255, 255, 0.25) 0%, rgba(255, 255, 255, 0.5) 100%)",
									...
									<tbody>
										{userReferrals?.pages?.flatMap((page: any) =>
											page?.referrals?.map((userReferral: any) => {
												...
												return (
													<tr key={userReferral._id} className="bg-white border-b">
														<td className="py-4 px-6 flex gap-3 items-center">
															<Image
																src={userReferral.expertDetails.photo || '/default-avatar.png'} // Add default image fallback
																width={100}
																height={100}
																alt="avatar"
															...
															<div>
																<div className="font-medium text-sm text-[#6B7280]">
																	{userReferral.expertDetails.fullName || 'Username'}
																</div>
															</div>
														</td>
														<td className="py-4 px-6 text-sm text-[#6B7280]">
															{formatDateTime(userReferral.expertDetails.createdAt).custom || 'N/A'}
														</td>
														...
														<td className="py-4 px-6 text-sm text-[#6B7280]">₹100</td>
													</tr>
												);
											})
										)}
