...
const ChatInterface: React.FC = () => {
    ...
    useUserStatus();
    const { audioStream, isRecording, audioBlob, startRecording, stopRecording, setAudioStream, mediaRecorderRef, setIsRecording } = useMediaRecorder();
    ...
    useEffect(() => {
        const fetchReceiverId = async () => {
            try {
                const currentUserChatsRef = doc(db, "userchats", user?.publicMetadata?.userId as string);
                ...
                if (currentUserChatsSnapshot.exists()) {
                    ...
                    const chat = currentUserChatsData.chats.find((c: { chatId: string | string[]; }) => c.chatId === chatId);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(doc(db, "userchats", receiverId), (docSnapshot) => {
